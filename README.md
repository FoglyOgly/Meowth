...
2. **Enable the !team command.** If you haven't already, create a role for each team. Enter the role names in the team_dict on line 50. The team emoji are configured on this line as well (see step 5 and 6).

3. **Enable the welcome message**. On lines 22 and 26, configure the names of your server's @admin role and #announcements or #welcome channel.

4. **Enable Google Maps hints.** In lines 30 and 31, replace "yourtown" and "yourstate" with your community's location. This makes the Google Maps location links work a lot better.
...
6. **Configure raid command strings.** Using the variables in lines 53-77, customize the emoji messages to match the ones on your server. Meowth will attempt to look up the names in your server's emoji list. You'll have to do this for each of the 25 custom emoji. You can also just use plain strings if your emoji slots are already taken or if you just prefer it that way.
