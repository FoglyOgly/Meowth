...
docker_setup_env() {
	file_env 'POSTGRES_PASSWORD' 'meowth'

	file_env 'POSTGRES_USER' 'meowth'
...
_main() {
	...
	if [ "$1" = 'postgres' ] && ! _pg_want_help "$@"; then
		...
		if [ -z "$DATABASE_ALREADY_EXISTS" ]; then
            ...
            sudo mv /app/database/schema.sql /app/database/schema.bak
            printf '%s: running %s\n' "$0" "/app/database/pokemon.sql"; PGHOST= PGHOSTADDR= pg_restore -U "$POSTGRES_USER" -d "$POSTGRES_DB" /app/database/pokemon.sql; printf '\n'
            sudo mv /app/database/pokemon.sql /app/database/pokemon.bak
            sudo mv /app/database/* /docker-entrypoint-initdb.d/
