...
async def print_raid_timer(channel):
    ...
    if not server_dict[channel.server]['raidchannel_dict'][channel]['active']:
        ...
        if server_dict[channel.server]['raidchannel_dict'][channel]['manual_timer']:
            timerstr += _("Meowth! This {raidtype} will {raidaction} at {expiry_time}!").format(raidtype=raidtype,raidaction=raidaction,expiry_time=strftime("%I:%M", localexpire),expiry_time24=strftime("%H:%M", localexpire))
        else:
