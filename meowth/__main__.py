    ...
    if (channel_exists == None) and (not Meowth.is_closed()):
        ...
        try:
            if (guild_dict[channel.guild.id]['raidchannel_dict'][channel.id]['active'] == False) and (not Meowth.is_closed()):
                for report_city, report_msg in guild_dict[guild.id]['raidchannel_dict'][channel.id]['reportcity'].items():
                    if dupechannel:
                        try:
                            report_channel = Meowth.get_channel(report_city)
                            reportmsg = await report_channel.fetch_message(report_msg)
            ...
            if guild_dict[message.guild.id]['raidchannel_dict'][message.channel.id]['active']:
                ...
                if "/maps" in message.content and "http" in message.content:
                    ...
                    try:
                        ...
                        pass
                    for report_city, report_msg in guild_dict[message.guild.id]['raidchannel_dict'][message.channel.id]['reportcity'].items():
                        report_channel = Meowth.get_channel(report_city)
                        oldreportmsg = await report_channel.fetch_message(report_msg)
                        try:
                             await oldreportmsg.edit(new_content=oldreportmsg.content, embed=newembed, content=oldreportmsg.content)
                        except:
                            pass
                        guild_dict[message.guild.id]['raidchannel_dict'][message.channel.id]['reportcity'][report_city] = oldreportmsg.id
...
async def changeraid(ctx, newraid):
    ...
    if newraid.isdigit():
        ...
        raid_img_url = 'https://raw.githubusercontent.com/witoldj/Meowth/master/images/eggs/{}?cache=0'.format(str(egg_img))
        raid_message = await channel.fetch_message(guild_dict[guild.id]['raidchannel_dict'][channel.id]['raidmessage'])
        report_city, report_msg = next(iter(guild_dict[guild.id]['raidchannel_dict'][channel.id]['reportcity'].items()))
        report_channel = Meowth.get_channel(report_city)
        report_message = await report_channel.fetch_message(report_msg)
        ...
        for report_city, report_msg in guild_dict[guild.id]['raidchannel_dict'][channel.id]['reportcity'].items():
            try:
                report_channel = Meowth.get_channel(report_city)
                report_message_old = await report_channel.fetch_message(report_msg)
                await report_message_old.edit(new_content=report_message.content, embed=raid_embed, content=report_message.content)
...
async def want(ctx,*,pokemon):
    ...
    if (len(want_list) == 1) and ((len(added_list) == 1) or (len(spellcheck_dict) == 1) or (len(already_want_list) == 1)):
        if len(added_list) == 1:
            ...
            want_img_url = 'https://raw.githubusercontent.com/witoldj/Meowth/master/images/pkmn/{0}_.png?cache=1'.format(str(want_number).zfill(3))
...
async def _wild(message, content):
    ...
    wild_img_url = 'https://raw.githubusercontent.com/witoldj/Meowth/master/images/pkmn/{0}_.png?cache=0'.format(str(wild_number).zfill(3))
...
async def _raid(ctx, content):
    ...
    raid_img_url = 'https://raw.githubusercontent.com/witoldj/Meowth/master/images/pkmn/{0}_.png?cache=0'.format(str(raid_number).zfill(3))
    ...
    raidreport = await message.channel.send(content=_('Meowth! {pokemon} raid reported by {member}! Details: {location_details}. Coordinate in {raid_channel}').format(pokemon=entered_raid.capitalize(), member=member_name, location_details=raid_details, raid_channel=raid_channel.mention), embed=report_embed)
    raid_reports = {message.channel.id: raidreport.id}
    await asyncio.sleep(1)
    raidmsg = _("{roletest}Meowth! {pokemon} raid reported by {member} in {citychannel}! Details: {location_details}. Coordinate here!\n\nClick the question mark reaction to get help on the commands that work in here.\n\nThis channel will be deleted five minutes after the timer expires.").format(roletest=roletest, pokemon=entered_raid.title(), member=member_name, citychannel=message.channel.mention, location_details=raid_details)
    raidmessage = await raid_channel.send(content=raidmsg, embed=raid_embed)
    await raidmessage.add_reaction('\u2754')
    await raidmessage.pin()
    if gyms:
        await asyncio.sleep(1)
        content = _('Meowth! {pokemon} raid reported by {member}! Details: {location_details}. Coordinate in {raid_channel}').format(pokemon=entered_raid.capitalize(), member=member_name, location_details=raid_details, raid_channel=raid_channel.mention)
        embed = report_embed
...
async def _raidegg(ctx, content):
    ...
    if (egg_level > 5) or (egg_level == 0):
        ...
        raid_embed.set_thumbnail(url=raid_img_url)
        member_name = message.author.nick if message.author.nick is not None else message.author.name
        raidreport = await message.channel.send(content=_('Meowth! Level {level} raid egg reported by {member}! Details: {location_details}. Coordinate in {raid_channel}').format(level=egg_level, member=member_name, location_details=raid_details, raid_channel=raid_channel.mention), embed=raid_embed)
        await asyncio.sleep(1)
        raid_reports = {message.channel.id: raidreport.id}
        raidmsg = _("Meowth! Level {level} raid egg reported by {member} in {citychannel}! Details: {location_details}. Coordinate here!\n\nClick the question mark reaction to get help on the commands that work in here.\n\nThis channel will be deleted five minutes after the timer expires.").format(level=egg_level, member=member_name, citychannel=message.channel.mention, location_details=raid_details)
        raidmessage = await raid_channel.send(content=raidmsg, embed=raid_embed)
        await raidmessage.add_reaction('\u2754')
        await raidmessage.pin()
        if gyms:
...
async def _eggassume(args, raid_channel, author=None):
    ...
    try:
        ...
        raid_message = None
    for report_city, report_msg in eggdetails['reportcity'].items():
        report_channel = Meowth.get_channel(report_city)
        egg_report = await report_channel.fetch_message(report_msg)
        try:
...
async def _eggtoraid(entered_raid, raid_channel, author=None):
    ...
    for report_city, report_msg in eggdetails['reportcity'].items():
        try:
            reportcitychannel = Meowth.get_channel(report_city)
        except (discord.errors.NotFound, AttributeError):
            pass
        if not reportcitychannel:
            continue
        try:
            egg_report = await reportcitychannel.fetch_message(report_msg)
        ...
        try:
            ...
            egg_report = None
        if egg_report is not None:
            report_city_list[report_city] = egg_report
...
async def _exraid(ctx, location):
    ...
    raid_channel_name += sanitize_channel_name(raid_details)
    raid_channel_name = "0👤" + raid_channel_name
    raid_channel_overwrite_list = list(channel.overwrites.items())
    ...
    await raidmessage.pin()
    guild_dict[message.guild.id]['raidchannel_dict'][raid_channel.id] = {
        'reportcity': {channel.id: raidreport.id},
        'trainer_dict': {
...
async def _invite(ctx):
    ...
    for channelid in rc_dict:
        ...
        if (rc_dict[channelid]['egglevel'] == 'EX') or (rc_dict[channelid]['type'] == 'exraid'):
            if guild_dict[guild.id]['configure_dict']['exraid']['permissions'] == "everyone" or (guild_dict[guild.id]['configure_dict']['exraid']['permissions'] == "same" and channel.id in rc_dict[channelid]['reportcity'].keys()):
...
async def research(ctx, *, details = None):
    ...
    research_embed = discord.Embed(colour=message.guild.me.colour).set_thumbnail(url='https://raw.githubusercontent.com/witoldj/Meowth/master/images/misc/field-research.png?cache=0')
...
async def _meetup(ctx, location):
    ...
    try:
        ...
        pass
    raid_img_url = 'https://raw.githubusercontent.com/witoldj/Meowth/master/images/misc/meetup.png?cache=0'
...
async def _timerset(raidchannel, exptime):
    ...
    try:
        ...
        pass
    for report_city, report_message in guild_dict[guild.id]['raidchannel_dict'][raidchannel.id]['reportcity'].items():
        report_channel = Meowth.get_channel(report_city)
        reportmsg = await report_channel.fetch_message(report_message)
        try:
...
async def starttime(ctx,*,start_time=""):
    ...
    if (start and now <= start) or timeset:
        ...
        try:
            ...
            pass
        for report_city, report_message in rc_d['reportcity'].items():
            report_channel = Meowth.get_channel(report_city)
            reportmsg = await report_channel.fetch_message(report_message)
            try:
                await reportmsg.edit(content=reportmsg.content,embed=embed)
...
async def _counters(ctx, pkmn, user = None, weather = None, movesetstr = "Unknown Moveset"):
    img_url = 'https://raw.githubusercontent.com/witoldj/Meowth/master/images/pkmn/{0}_.png?cache=4'.format(str(get_number(pkmn)).zfill(3))
...
@Meowth.command(aliases=['i', 'maybe', 'może', 'moze', 'zainteresowany'])
@checks.activechannel()
...
@Meowth.command(aliases=['c', 'idę', 'ide', 'wdrodze', 'jadę', 'jade'])
@checks.activechannel()
...
@Meowth.command(aliases=['h', 'namiejscu', 'jestem', 'czekam'])
@checks.activechannel()
...
async def _edit_party(channel, author=None):
    ...
    for report_city, report_message in guild_dict[channel.guild.id]['raidchannel_dict'][channel.id]['reportcity'].items():
        reportchannel = Meowth.get_channel(report_city)
        try:
            reportmsg = await reportchannel.fetch_message(report_message)
        except:
            pass
        try:
...
@Meowth.command(aliases=['x', 'anuluj'])
@checks.raidchannel()
